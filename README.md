<p align="center">
  <img src="https://camo.githubusercontent.com/b16ecdcac9c3d21ec3a49459430f747b46b3a37acc95ee468d87d0ec61ff2392/68747470733a2f2f692e696d6775722e636f6d2f576d4d6e5352742e706e67">
</p>

# [IS334.Q13.TMĐT] - ĐỒ ÁN XÂY DỰNG WEBSITE KẾT NỐI BẠN BÈ FINDMATE

##  Giới thiệu

**Find Your Mate** là một nền tảng giúp bạn **kết nối với người khác để tham gia các hoạt động xã hội, thể thao hoặc học tập**.  
Bạn cảm thấy cô đơn khi chơi bóng chuyền một mình? Muốn tìm người đi xem phim, học nhóm, ăn uống hay đơn giản là chill cùng nhau?  
Hãy để chúng tôi giúp bạn **"Find your mate, find your vibe"**.


---

## Social link
* **Facebook:** https://www.facebook.com/people/FindMate/61581945898819/
* **Tiktok:**   https://www.tiktok.com/@findmate8?_t=ZS-90NRUiEduCb&_r=1
* **Website:**  https://find-mate-website.vercel.app/

## Thông tin dự án
* **Trường:** Trường Đại học Công nghệ Thông tin, ĐHQG-HCM  
* **Khoa:** Hệ thống thông tin  
* **Môn học:** Thương mại điện tử (IS334)  
* **Giảng viên hướng dẫn:** Ts. Liêu Nguyễn Ngọc Quế
* **Nhóm sinh viên thực hiện:** Nhóm CellphoneS

## Danh sách thành viên
| STT | Họ tên | MSSV | Chức vụ |
|:---:|---|:---:|:---:|
| 1 | Lã Đăng Khoa | 23520739 | Nhóm trưởng |
| 2 | Lê Thái Khang | 23520691 | Thành viên |



## Công nghệ sử dụng
* **Ngôn ngữ:** HTML, CSS, JavaScript  
* **Thiết kế giao diện:** Figma  
* **Triển khai:** Chạy trực tiếp file HTML, Live Server (VS Code)  hoặc Vercel


## Hướng dẫn cài đặt 
1. **Clone repository**:
```bash
https://github.com/KOALAFixBug/FindMate-Website.git
```
2. **Di chuyển vào thư mục**:
```bash
cd Findmate_Website
```
3. **Chạy dự án**

**Cách 1:** Mở trực tiếp file HTML

Chỉ cần tìm đến file **index.html** (hoặc file HTML chính của bạn) trong thư mục dự án.

Nháy đúp chuột vào file đó, trang web sẽ tự động mở lên bằng trình duyệt web mặc định của bạn.

**Cách 2:** Sử dụng **"Live Server"** (Khuyên dùng cho phát triển) Nếu bạn sử dụng trình soạn thảo code như Visual Studio Code, cách tốt nhất là dùng extension "Live Server":

Cài đặt extension Live Server từ VS Code Marketplace.

Sau khi cài đặt, bạn chỉ cần nhấn chuột phải vào file index.html và chọn **"Open with Live Server"**.

Trang web sẽ tự động mở trong trình duyệt và tự động tải lại mỗi khi bạn lưu file, giúp việc code nhanh hơn rất nhiều.

**Cách 3:** Hoặc truy cập thẳng vào trang web: https://find-mate-website.vercel.app/




##  Đóng góp

Chúng tôi luôn hoan nghênh các ý tưởng, phản hồi và đóng góp từ các bạn!  






